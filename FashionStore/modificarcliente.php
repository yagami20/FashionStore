...
$U ->setemail ($cor);
 ...
 if(!$errores){
    if( $U->modificar()){
 echo ("<script> alert('Registro Modoficado'); location.href = 'reporteclientes.php';</script>");
} else {
echo ("<scrip> alert('Error, no se puede Modificar'); location.href = 'reporteclientes.php';</scrip>");
}
}
else{
$errorya=$errores[0];
